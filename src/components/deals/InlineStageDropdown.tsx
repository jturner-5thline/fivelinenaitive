    ...
    <DropdownMenu open={isMenuOpen} onOpenChange={(open) => {
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          <Badge
            variant="outline"
            className={`deal-stage-chip relative inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold leading-tight cursor-pointer max-w-full text-left whitespace-nowrap border border-cyan-300/70 text-cyan-200 !bg-transparent transition-colors hover:border-cyan-200 hover:text-cyan-100 ${className}`}
          >
            {currentConfig.label}
          </Badge>
        </button>
