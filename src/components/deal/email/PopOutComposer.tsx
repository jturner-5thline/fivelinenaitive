...
export function PopOutComposer({ draft: initialDraft, onSend, onDiscard, onPopIn }: PopOutComposerProps) {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <Button variant="ghost" size="icon" className="h-5 w-5 hover:text-destructive" onClick={(e) => { e.stopPropagation(); onDiscard(); }}>
            <X className="h-3 w-3" />
          </Button>
        </div>
      </div>
    ...
    >
      ...
      <div className="px-3 py-2 space-y-1.5 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <Input value={to} onChange={e => setTo(e.target.value)} placeholder="recipient@example.com" className="h-7 text-xs border-0 border-b rounded-none focus-visible:ring-0 px-0 bg-transparent" />
        ...
        {showCcBcc && (
          <>
            <div className="flex items-center gap-2">
              ...
              <Input value={cc} onChange={e => setCc(e.target.value)} className="h-7 text-xs border-0 border-b rounded-none focus-visible:ring-0 px-0 bg-transparent" />
            </div>
            <div className="flex items-center gap-2">
              <Label className="text-[11px] text-muted-foreground w-8 shrink-0">Bcc</Label>
              <Input value={bcc} onChange={e => setBcc(e.target.value)} className="h-7 text-xs border-0 border-b rounded-none focus-visible:ring-0 px-0 bg-transparent" />
            </div>
          </>
        )}
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-8 shrink-0">Subj</Label>
          <Input value={subject} onChange={e => setSubject(e.target.value)} className="h-7 text-xs border-0 border-b rounded-none focus-visible:ring-0 px-0 bg-transparent font-medium" />
        </div>
      </div>
