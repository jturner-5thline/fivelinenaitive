...
import {
  ...
  Search, Highlighter, Palette, X, Replace,
  Plus, Trash2, Keyboard, ChevronDown, Type,
  Printer, FileDown,
} from 'lucide-react';
...
// ─── Ribbon Icon Button ───
function RibbonBtn({
  onClick, isActive, icon: Icon, label, disabled, className,
}: {
...
function ColorSwatchPicker({
  currentColor, onColorChange, icon: Icon, label, underlineColor,
}: {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="inline-flex items-center justify-center h-7 px-1 rounded-sm hover:bg-accent/80 transition-colors relative"
          onMouseDown={(e) => e.preventDefault()}
        >
          <Icon className="h-3.5 w-3.5" />
      ...
      <PopoverContent className="w-auto p-2" align="start" sideOffset={4}>
        ...
        <div className="grid grid-cols-10 gap-0.5">
          {COLORS.map(color => (
            <button
              key={color}
              className={cn(
                "h-[18px] w-[18px] rounded-sm border border-border/50 hover:scale-125 transition-transform",
                currentColor === color && "ring-1.5 ring-primary ring-offset-1",
              )}
        ...
        <button
          className="w-full text-xs text-muted-foreground hover:text-foreground mt-1.5 py-1 rounded hover:bg-accent/50 transition-colors"
          onMouseDown={(e) => e.preventDefault()}
...
export function DealSpaceNoteEditor({ note, onUpdate, onDownload }: DealSpaceNoteEditorProps) {
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      if (html !== lastSavedContentRef.current) debouncedSave(html);
    };
  ...
  const handleReplace = () => {
    ...
    const html = editor.getHTML();
    editor.commands.setContent(html.replace(findText, replaceText));
  };
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center gap-1 px-2 py-0.5 text-xs border-b border-border/40">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="px-2 py-1 rounded hover:bg-accent/60 transition-colors text-muted-foreground hover:text-foreground">File</button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="min-w-[180px]">
            <DropdownMenuItem onClick={() => onDownload(note)}>
              <FileDown className="h-3.5 w-3.5 mr-2" /> Download as .docx
            </DropdownMenuItem>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="px-2 py-1 rounded hover:bg-accent/60 transition-colors text-muted-foreground hover:text-foreground">Edit</button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="min-w-[180px]">
            <DropdownMenuItem onClick={() => editor.chain().focus().undo().run()}>
              <Undo className="h-3.5 w-3.5 mr-2" /> Undo <span className="ml-auto text-muted-foreground text-[10px]">⌘Z</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => editor.chain().focus().redo().run()}>
              <Redo className="h-3.5 w-3.5 mr-2" /> Redo <span className="ml-auto text-muted-foreground text-[10px]">⌘Y</span>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => setShowFindReplace(true)}>
              <Search className="h-3.5 w-3.5 mr-2" /> Find and replace <span className="ml-auto text-muted-foreground text-[10px]">⌘F</span>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="px-2 py-1 rounded hover:bg-accent/60 transition-colors text-muted-foreground hover:text-foreground">Insert</button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="min-w-[180px]">
            ...
            <DropdownMenuItem onClick={() => { setLinkUrl(editor.getAttributes('link').href || ''); setShowLinkDialog(true); }}>
              <LinkIcon className="h-3.5 w-3.5 mr-2" /> Link
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => editor.chain().focus().setHorizontalRule().run()}>
              <Minus className="h-3.5 w-3.5 mr-2" /> Horizontal line
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="px-2 py-1 rounded hover:bg-accent/60 transition-colors text-muted-foreground hover:text-foreground">Format</button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="min-w-[180px]">
            <DropdownMenuItem onClick={() => editor.chain().focus().toggleBold().run()}>
              <Bold className="h-3.5 w-3.5 mr-2" /> Bold <span className="ml-auto text-muted-foreground text-[10px]">⌘B</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => editor.chain().focus().toggleItalic().run()}>
              <Italic className="h-3.5 w-3.5 mr-2" /> Italic <span className="ml-auto text-muted-foreground text-[10px]">⌘I</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => editor.chain().focus().toggleUnderline().run()}>
              <UnderlineIcon className="h-3.5 w-3.5 mr-2" /> Underline <span className="ml-auto text-muted-foreground text-[10px]">⌘U</span>
            </DropdownMenuItem>
            ...
            <DropdownMenuItem onClick={() => editor.chain().focus().toggleBlockquote().run()}>
              <Quote className="h-3.5 w-3.5 mr-2" /> Block quote
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="px-2 py-1 rounded hover:bg-accent/60 transition-colors text-muted-foreground hover:text-foreground">Tools</button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="min-w-[180px]">
            ...
            <DropdownMenuItem onClick={() => setShowFindReplace(true)}>
              <Search className="h-3.5 w-3.5 mr-2" /> Find and replace
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <div className="flex items-center gap-0.5 px-2 py-1 border-b border-border/40 bg-muted/20">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="inline-flex items-center gap-1 h-7 px-2 rounded-sm text-xs hover:bg-accent/80 transition-colors min-w-[100px] justify-between border border-border/40"
              onMouseDown={(e) => e.preventDefault()}
            >
        ...
        <ColorSwatchPicker
          ...
          onColorChange={(c) => c ? editor.chain().focus().setColor(c).run() : editor.chain().focus().unsetColor().run()}
          icon={Type}
          label="Text color"
          underlineColor={editor.getAttributes('textStyle').color || '#000000'}
        />
        <ColorSwatchPicker
          currentColor={editor.getAttributes('highlight').color}
          onColorChange={(c) => c ? editor.chain().focus().toggleHighlight({ color: c }).run() : editor.chain().focus().unsetHighlight().run()}
          icon={Highlighter}
          label="Highlight color"
          underlineColor={editor.getAttributes('highlight').color || '#ffff00'}
        />
        <RibbonDivider />

        {/* Link */}
        <RibbonBtn
          onClick={() => { setLinkUrl(editor.getAttributes('link').href || ''); setShowLinkDialog(true); }}
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button
              className={cn(
                "inline-flex items-center justify-center h-7 w-7 rounded-sm transition-colors hover:bg-accent/80",
          ...
          <PopoverContent className="w-auto p-2" align="start" sideOffset={4}>
            ...
            <div className="flex flex-col gap-0.5">
              <button className="flex items-center gap-2 text-xs px-2 py-1.5 rounded hover:bg-accent/60 transition-colors text-left" onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()}>
              ...
              {editor.isActive('table') && (
                <>
                  ...
                  <button className="flex items-center gap-2 text-xs px-2 py-1.5 rounded hover:bg-accent/60 text-left text-destructive" onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().deleteTable().run()}>
      ...
      {showFindReplace && (
        <div className="flex items-center gap-2 px-3 py-1.5 border-b border-border/40 bg-muted/30">
          ...
          <button className="h-7 w-7 inline-flex items-center justify-center rounded-sm hover:bg-accent/80" onClick={() => setShowFindReplace(false)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid grid-cols-2 gap-y-2 text-sm">
            {[
              ...
              ['⌘+Z', 'Undo'], ['⌘+Y', 'Redo'], ['⌘+F', 'Find & Replace'],
              ['⌘+Shift+7', 'Numbered list'], ['⌘+Shift+8', 'Bullet list'],
              ['⌘+Shift+9', 'Checklist'], ['Tab', 'Indent'], ['Shift+Tab', 'Outdent'],
              ['Enter', 'New paragraph'], ['---', 'Horizontal rule'],
            ].map(([key, desc]) => (
