...
import {
  ...
  GripVertical, Bold, Italic, Type, List, Image, Table2,
  FileDown, Printer
} from 'lucide-react';
...
export function ReportEditor({ dealName, dealStage, dealValue, metrics, statements, className }: ReportEditorProps) {
  ...
  const exportToDocx = useCallback(async () => {
    try {
      ...
      const children: any[] = [
        new Paragraph({
          text: `${dealName || 'Deal'} — Screening Memo`,
          heading: HeadingLevel.TITLE,
        }),
        new Paragraph({
          children: [
            new TextRun({ text: `${dealStage || 'Screening'} • ${dealValue ? `$${(dealValue / 1e6).toFixed(1)}MM` : 'TBD'} • ${new Date().toLocaleDateString()}`, italics: true, size: 20 }),
          ],
        }),
        new Paragraph({ text: '' }),
      ];
                ...
                {!section.collapsed && (
                  <div className="pl-6 mt-2">
                    ...
                    <textarea
                      ...
                      placeholder="Write your analysis here, or use the AI chat to generate content and click 'Add to report'..."
                    />
                  </div>
                )}
              </div>
