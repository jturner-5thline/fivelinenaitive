...
import {
  BUILTIN_SNAPSHOT_FIELDS, CustomCompanyField, CustomCompanyFieldType,
  useCompanySnapshotFieldConfig,
} from '@/hooks/useCompanySnapshotFieldConfig';

const TYPE_LABELS: Record<CustomCompanyFieldType, string> = {
  text: 'Free text',
...
export function ManageCompanyFieldsDialog({ open, onOpenChange }: Props) {
  const { config, save } = useCompanySnapshotFieldConfig();
  const [disabled, setDisabled] = useState<Set<string>>(new Set());
  const [custom, setCustom] = useState<CustomCompanyField[]>([]);
  ...
  const addCustom = () => {
    setCustom(prev => [...prev, { key: `field_${Date.now()}`, label: 'New Field', type: 'text', order: prev.length }]);
  };

  const updateCustom = (idx: number, patch: Partial<CustomCompanyField>) => {
  ...
  const handleSave = async () => {
    ...
    for (const f of custom) {
      ...
      seen.add(key);
      if (f.type !== 'text' && (!f.options || f.options.filter(o => o.trim()).length === 0)) {
    ...
    try {
      await save({
        ...
        custom: custom.map((f, i) => ({
          ...
          order: i,
          options: f.type === 'text' ? undefined : (f.options || []).map(o => o.trim()).filter(Boolean),
        })),
      });
      toast.success('Snapshot fields updated');
      onOpenChange(false);
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Manage Snapshot Fields</DialogTitle>
          <DialogDescription>
            Show or hide standard fields and add your own. Changes apply to every company in this workspace.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <div className="space-y-1">
                          ...
                          <Select value={f.type} onValueChange={(v) => updateCustom(idx, { type: v as CustomCompanyFieldType })}>
