...
export function useDealWriteup(dealId: string | undefined) {
  ...
  const saveWriteup = useCallback(async (data: DealWriteUpData): Promise<boolean> => {
    ...
    try {
      const writeupData = {
        ...
        industry: data.industries?.join(', ') || null,
        location: data.location || null,
        year_founded: data.yearFounded || null,
        customer_base: data.customerBase?.join(', ') || null,
        headcount: data.headcount || null,
        deal_type: data.dealTypes?.join(', ') || null,
        billing_model: data.billingModels?.join(', ') || null,
  ...
  const rowToData = useCallback((row: DealWriteupRow | null): DealWriteUpData | null => {
    ...
    let financialComments: FinancialComment[] = [];
    if ((row as any).financial_comments && Array.isArray((row as any).financial_comments)) {
      financialComments = (row as any).financial_comments as unknown as FinancialComment[];
    }

    // Parse team from JSON
    let team: TeamMember[] = [];
    if ((row as any).team && Array.isArray((row as any).team)) {
      team = (row as any).team as unknown as TeamMember[];
    }
    ...
    return {
      ...
      industries: row.industry ? row.industry.split(', ').filter(Boolean) : [],
      location: row.location || '',
      yearFounded: (row as any).year_founded || '',
      customerBase: (row as any).customer_base ? (row as any).customer_base.split(', ').filter(Boolean) : [],
      headcount: (row as any).headcount || '',
      dealTypes: row.deal_type ? row.deal_type.split(', ').filter(Boolean) : [],
      billingModels: row.billing_model ? row.billing_model.split(', ').filter(Boolean) : [],
