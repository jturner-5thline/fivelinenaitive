...
// (e.g. Xnergy United Network went lenders-in-review → terms-issued), so
// we treat first entry into any of these downstream stages as "signed" and
// dedupe to first entry per deal in useStageEntryMetric.
const SIGNED_STAGES: string[] = [
  'final-credit-items',
  'terms-issued',
  'in-due-diligence',
  'funded-invoiced',
...
export function usePipelineStageMetrics(quarter: QuarterOption): PipelineMetrics {
  ...
  const debtDealsSigned = useStageEntryMetric(SIGNED_STAGES, quarter, DEBT_STAGE_PIPELINES, { firstEverInWindow: true, historyPipelineIds: [ACTIVE_PIPELINE_ID] });
...
export function useConsolidatedDebtPipelineMetrics(
  ...
  const finalCreditItemsTrend = useStageEntryTrendSeries(SIGNED_STAGES, todayAnchor, ACTIVE_PIPELINE_ID);
  ...
  const finalCreditItemsRolling6 = useStageEntryMetric(SIGNED_STAGES, sixMonthPeriod, DEBT_STAGE_PIPELINES, { firstEverInWindow: true, historyPipelineIds: [ACTIVE_PIPELINE_ID] });
  const fundedInvoicedRolling6 = useStageEntryMetric(CLOSED_STAGES, sixMonthPeriod, ACTIVE_PIPELINE_ID);
  const finalCreditItemsRolling12 = useStageEntryMetric(SIGNED_STAGES, twelveMonthPeriod, DEBT_STAGE_PIPELINES, { firstEverInWindow: true, historyPipelineIds: [ACTIVE_PIPELINE_ID] });
  ...
  const finalCreditItemsRolling6Prior = useStageEntryMetric(SIGNED_STAGES, priorSixMonthPeriod, DEBT_STAGE_PIPELINES, { firstEverInWindow: true, historyPipelineIds: [ACTIVE_PIPELINE_ID] });
  const fundedInvoicedRolling6Prior = useStageEntryMetric(CLOSED_STAGES, priorSixMonthPeriod, ACTIVE_PIPELINE_ID);
  const finalCreditItemsRolling12Prior = useStageEntryMetric(SIGNED_STAGES, priorTwelveMonthPeriod, DEBT_STAGE_PIPELINES, { firstEverInWindow: true, historyPipelineIds: [ACTIVE_PIPELINE_ID] });
