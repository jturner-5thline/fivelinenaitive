...
export function useContextualSuggestions(deal: DealData | null) {
  ...
  const computedSuggestions = useMemo(() => {
    ...
    if (suggestionPrefs.lendersWithoutNotes) {
    ...
    // 7. No milestones set
    if (suggestionPrefs.noMilestones && (!deal.milestones || deal.milestones.length === 0)) {
      newSuggestions.push({
        id: 'no-milestones',
        type: 'action',
        priority: 'low',
    ...
    if (suggestionPrefs.allMilestonesComplete && deal.milestones && deal.milestones.length > 0 && deal.milestones.every(m => m.completed)) {
      newSuggestions.push({
