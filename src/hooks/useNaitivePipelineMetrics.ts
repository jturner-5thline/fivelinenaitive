...
export function useNaitivePipelineMetrics(deals: Deal[], stages: DealStageOption[]) {
  ...
  const agingData = useMemo<StageAgingItem[]>(() => {
    const now = new Date();
    return stages
      .filter(s => s.id !== 'closed-won' && s.id !== 'closed-lost')
      .map(s => {
        const stageDeals = deals.filter(d => d.stage === s.id);
  ...
  const healthMix = useMemo<HealthMixItem[]>(() => {
    ...
    return statuses.map(s => ({
      ...s,
      count: deals.filter(d => d.status === s.status && d.stage !== 'closed-won' && d.stage !== 'closed-lost').length,
    }));
  }, [deals]);
  ...
  const trendData = useMemo<PipelineTrendPoint[]>(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const won = deals.filter(d => d.stage === 'closed-won' && new Date(d.updatedAt) >= monthStart && new Date(d.updatedAt) <= monthEnd).length;
  ...
  const recommendations = useMemo<NaitivePipelineRecommendation[]>(() => {
    ...
    const staleProspects = activeDeals.filter(d => d.stage === 'prospect' && differenceInDays(now, new Date(d.updatedAt)) >= 7);
    ...
    stages.filter(s => s.id !== 'closed-won' && s.id !== 'closed-lost').forEach(s => {
    ...
    const closingDeals = activeDeals.filter(d => d.stage === 'closing');
    if (closingDeals.length > 0) {
      recs.push({
        id: 'closing-push',
        message: `${closingDeals.length} deal${closingDeals.length > 1 ? 's' : ''} in Closing stage — push to close this period.`,
