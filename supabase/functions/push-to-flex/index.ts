  ...
  try {
    ...
    if (action === "unpublish") {
      ...
      const flexDeal = {
        ...
        existing_debt: resolveExistingDebt(writeUpData!.existingDebtDetails, existingDebtItems) ?? null,
        data_room_url: nonEmpty(writeUpData!.dataRoomUrl),
        // Always send arrays (even empty) so FLEx clears removed sections instead of keeping stale data.
        key_items: writeUpData!.keyItems ?? [],
        company_highlights: writeUpData!.companyHighlights ?? [],
        financial_years: writeUpData!.financialYears ?? [],
        financial_comments: writeUpData!.financialComments ?? [],
        cap_table: (freshOwnership ?? []).map(o => ({
          name: o.owner_name,
          ownership: Number(o.ownership_percentage),
          url: o.owner_url || undefined,
        })),
        total_equity_raised: writeUpData!.totalEquityRaised || null,
        is_published: !writeUpData!.publishAsAnonymous,
        team: writeUpData!.team ?? [],
