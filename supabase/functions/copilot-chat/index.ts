...
- Due date: parse natural-language dates ("today", "tomorrow", "Friday", "next Tuesday", "in two weeks", "Mar 15") into a YYYY-MM-DD string and pass as due_date. If the user gave no date at all, omit due_date — the task will be created with no due date. If the date is genuinely AMBIGUOUS (e.g. "Tuesday" when both this and next Tuesday are plausible, or a date that has already passed this year), ask ONE short clarifying question before calling create_task. Do not guess.
