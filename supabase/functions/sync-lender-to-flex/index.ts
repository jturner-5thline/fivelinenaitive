...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Single lender sync to FLEx requested for lender: ${lender_id} by user: ${user.id}`);

    // Fetch the specific lender
    ...
    const flexLender = {
      id: l.id,
      naitive_lender_id: l.id, // Include Naitive ID for mapping
      flex_lender_id: l.flex_lender_id, // Include Flex ID if we have it
      name: l.name,
      email: l.email,
      ...
      sync_source: "naitive", // Mark this as coming from Naitive
    };
    ...
    console.log(`Sending lender "${l.name}" to FLEx`);
    ...
    console.log(`FLEx single lender sync response (${flexResponse.status}):`, responseText);

    if (!flexResponse.ok) {
      console.error(`FLEx lender sync error: ${flexResponse.status} - ${responseText}`);
      
      // Parse the FLEx response for better error handling
      let flexError;
      try {
        flexError = JSON.parse(responseText);
      } catch {
    ...
    console.log(`Successfully synced lender "${l.name}" to FLEx`);

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `Successfully synced "${l.name}" to FLEx`,
        lender_id: l.id,
        lender_name: l.name,
    ...
    console.error("Error in sync-lender-to-flex function:", error);
    return new Response(
