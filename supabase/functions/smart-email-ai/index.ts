...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version, x-suppress-error-toast",
};
...
Analyze this thread and create a follow-up sequence plan. Consider the deal stage, lender relationships, and urgency.`;
    ...
    if (action === "generate_draft_options" && typeof parsed === "object" && !parsed.raw) {
      ...
      // Signature append removed — the composer is the single source of truth
      // for the user's signature. Appending here was causing the recipient
      // to see the signature twice (once from the composer body, once from
      // this server-side append). The composer injects the saved signature
      // when the AI draft lands (see EmailComposerCard's AI ready handler).
    }
