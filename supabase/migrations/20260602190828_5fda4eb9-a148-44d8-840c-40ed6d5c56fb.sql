...
DECLARE
  ...
  -- normalized (lower, no spaces/punct) allowlist incl. aliases
...
BEGIN
  ...
  scored AS (
    SELECT
      b.*,
      -- allowlist hit (exact normalized OR strong trigram similarity)
      (
        b.norm_name = ANY(norm_allow)
        OR EXISTS (
          SELECT 1 FROM unnest(norm_allow) AS a
          WHERE extensions.similarity(b.norm_name, a) >= 0.9
        )
      ) AS in_allowlist,
      (
        lower(coalesce(b.deal_owner,'')) LIKE '%niki%'
        OR lower(coalesce(b.deal_owner,'')) LIKE '%heikali%'
      ) AS legacy_owner_hit
    FROM base b
  ),
  candidates AS (
    ...
    WHERE (in_allowlist OR legacy_owner_hit)
      -- exclusion: owner already set
      AND deal_owner_user_id IS NULL
      -- exclusion: closed/funded/lost > 365 days ago
      AND COALESCE(closed_at, lost_at, '1970-01-01'::timestamptz) > (now() - INTERVAL '365 days')
  ),
  enriched AS (
    SELECT
      c.id AS deal_id,
      c.company AS deal_name,
      c.deal_owner AS current_owner_text,
      c.deal_owner_user_id AS current_owner_uuid,
      c.manager AS current_manager,
      c.status AS current_status,
      c.stage AS current_stage,
      c.closed_at AS current_closed_at,
      c.lost_at AS current_lost_at,
      c.terms_issued_at AS current_terms_issued_at,
      c.terms_signed_at AS current_terms_signed_at,
      c.updated_at,
      c.in_allowlist,
      c.legacy_owner_hit,
      c.name_dupe_count,
  ...
  classified AS (
    SELECT
      e.*,
      CASE
        WHEN in_allowlist THEN 1.0
        WHEN legacy_owner_hit THEN 0.95
        ELSE 0.6
      END AS match_confidence,
      CASE WHEN regexp_replace(lower(deal_name),'[^a-z0-9]','','g') = 'evgateway'
            AND current_lost_at IS NULL
        THEN COALESCE(proposed_lost_at_from_history, updated_at)
        ELSE NULL
      END AS proposed_lost_at,
      CASE WHEN current_terms_issued_at IS NULL AND proposed_terms_issued_at IS NOT NULL
        THEN proposed_terms_issued_at ELSE NULL
      END AS proposed_terms_issued,
      (regexp_replace(lower(deal_name),'[^a-z0-9]','','g') = 'lango'
        AND current_closed_at IS NULL) AS lango_review_blocked
    FROM enriched e
  )
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'proposed_owner', rep_name,
    'proposed_owner_user_id', rep_user_id,
    'current_status', current_status,
    'proposed_status', CASE
      WHEN proposed_lost_at IS NOT NULL THEN 'lost'
    ...
    'notes', CONCAT_WS(' | ',
      ...
      CASE WHEN name_dupe_count > 1
            AND regexp_replace(lower(deal_name),'[^a-z0-9]','','g') = 'evgateway'
        THEN 'possible duplicate — review (' || name_dupe_count || ' rows under this name)'
        ELSE NULL END,
  ...
  -- BBP candidate matches: top 5 by trigram similarity, regardless of allowlist/exclusion
  ...
  FROM (
    SELECT
      ...
      d.stage,
      d.status,
      ROUND(extensions.similarity(regexp_replace(lower(coalesce(d.company,'')),'[^a-z0-9]','','g'), 'bbp')::numeric, 3) AS score
    FROM public.deals d
    WHERE lower(coalesce(d.company,'')) NOT LIKE 'test %'
      AND lower(coalesce(d.company,'')) NOT LIKE 'test-%'
    ORDER BY extensions.similarity(regexp_replace(lower(coalesce(d.company,'')),'[^a-z0-9]','','g'), 'bbp') DESC NULLS LAST
    LIMIT 5
  ) t;
  ...
  summary := jsonb_build_object(
    ...
    'allowlist_canonical', to_jsonb(canonical),
    'generated_at', now()
  );
  ...
  result := jsonb_build_object(
    ...
    'bbp_candidates', COALESCE(bbp_json,'[]'::jsonb)
  );
