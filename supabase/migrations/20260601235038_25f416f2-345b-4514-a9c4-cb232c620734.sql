...
-- Patch: include deal_class in the deals insert (force 'standard' for demo clone).
...
BEGIN
  ...
  -- Idempotency: wipe target rows we manage.
  ...
  -- Master lenders sample with anonymized PII
  ...
  -- Deals sample with id remap (force deal_class='standard' so 5th Line gates stay clean)
  ...
  ins AS (
    INSERT INTO deals(
      ...
      closing_date, sourced_via, deal_class, dashboard_closing_date, lead_source,
      referral_source, opportunity_type, services_offered, fee_type, mrr, one_time_revenue,
    ...
    SELECT
      ...
      src.closing_date, src.sourced_via, 'standard', src.dashboard_closing_date, src.lead_source,
  ...
  -- Deal stage history for cloned deals
