-- Ensure every newly created workspace gets an Active Pipeline before any sample-deal seeding runs.
...
BEGIN
  ...
  SELECT
    ...
    0,
    jsonb_build_array(
      jsonb_build_object('id', 'prospect', 'label', 'Prospect', 'color', 'bg-slate-500'),
      jsonb_build_object('id', 'qualification', 'label', 'Qualification', 'color', 'bg-blue-500'),
      jsonb_build_object('id', 'proposal', 'label', 'Proposal', 'color', 'bg-yellow-500'),
      jsonb_build_object('id', 'negotiation', 'label', 'Negotiation', 'color', 'bg-orange-500'),
      jsonb_build_object('id', 'closed-won', 'label', 'Closed Won', 'color', 'bg-green-500'),
      jsonb_build_object('id', 'closed-lost', 'label', 'Closed Lost', 'color', 'bg-red-500')
    )
  WHERE NOT EXISTS (
    SELECT 1
...
BEGIN
  ...
  -- Keep the rest of the existing default seeding behavior unchanged.
END;
$function$;