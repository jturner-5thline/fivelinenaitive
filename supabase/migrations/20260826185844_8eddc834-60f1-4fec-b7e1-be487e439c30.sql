...
-- Shared definition: qualifying events (lender added OR primary contact info
-- changed) + attachment to a deal as a funding source within 2 weeks.
...
-- Range-based variant powering the "New Qualified Lenders" KPI tile.
...
REVOKE ALL ON FUNCTION public.get_new_qualified_lenders(uuid, timestamptz, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_new_qualified_lenders(uuid, timestamptz, timestamptz) TO authenticated, service_role;
