...
BEGIN
  ...
  -- UPDATE
  v_actor := COALESCE(auth.uid(), NEW.updated_by, NEW.assigned_to, NEW.created_by, OLD.assigned_to);
  IF v_actor IS NULL THEN RETURN NEW; END IF;
...
DROP TRIGGER IF EXISTS trg_log_task_assignment_status ON public.tasks;
CREATE TRIGGER trg_log_task_assignment_status
AFTER INSERT OR UPDATE OF status, assigned_to ON public.tasks
FOR EACH ROW EXECUTE FUNCTION public.log_task_assignment_status_activity();
