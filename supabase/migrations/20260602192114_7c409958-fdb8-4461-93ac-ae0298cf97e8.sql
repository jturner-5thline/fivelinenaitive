...
BEGIN
  ...
  SELECT email, COALESCE(raw_user_meta_data->>'full_name', email)
    INTO rep_email, rep_name FROM auth.users WHERE id = rep_user_id;

  WITH base AS (
    SELECT d.*,
      regexp_replace(lower(coalesce(d.company,'')), '[^a-z0-9]', '', 'g') AS norm_name,
      lower(coalesce(d.company,'')) AS lower_name,
  ...
  scored AS (
    SELECT b.*,
      (
        ...
        OR EXISTS (SELECT 1 FROM unnest(norm_allow) AS a WHERE public.similarity(b.norm_name, a) >= 0.9)
        OR EXISTS (
          SELECT 1 FROM unnest(norm_allow) AS a
          WHERE length(a) >= 4 AND b.lower_name ~ ('\m' || a || '\M')
        )
      ) AS in_allowlist,
  ...
  enriched AS (
    SELECT c.id AS deal_id, c.company AS deal_name, c.deal_owner AS current_owner_text,
      ...
      (SELECT MIN(entered_at) FROM public.deal_stage_durations dsd
        WHERE dsd.deal_id = c.id AND dsd.stage_slug IN ('terms-issued','final-credit-items','submitted-to-lenders')
      ) AS proposed_terms_issued_at,
      (SELECT MAX(entered_at) FROM public.deal_stage_durations dsd
        WHERE dsd.deal_id = c.id AND dsd.stage_slug IN ('closed-lost','lost','withdrawn')
  ...
  bucketed AS (
    -- Compute fiscal buckets inline as scalars (avoid the SRF helper inside CASE)
    SELECT c.*,
      CASE
        WHEN c.current_closed_at IS NOT NULL THEN
          'FY' || EXTRACT(YEAR FROM c.current_closed_at)::int || ' Q' || EXTRACT(QUARTER FROM c.current_closed_at)::int
        WHEN c.current_lost_at IS NOT NULL THEN
          'FY' || EXTRACT(YEAR FROM c.current_lost_at)::int || ' Q' || EXTRACT(QUARTER FROM c.current_lost_at)::int
        ELSE NULL
      END AS current_fiscal_bucket_str,
      CASE
        WHEN c.current_closed_at IS NOT NULL THEN
          'FY' || EXTRACT(YEAR FROM c.current_closed_at)::int || ' Q' || EXTRACT(QUARTER FROM c.current_closed_at)::int
        WHEN c.proposed_lost_at IS NOT NULL THEN
          'FY' || EXTRACT(YEAR FROM c.proposed_lost_at)::int || ' Q' || EXTRACT(QUARTER FROM c.proposed_lost_at)::int
        WHEN c.current_lost_at IS NOT NULL THEN
          'FY' || EXTRACT(YEAR FROM c.current_lost_at)::int || ' Q' || EXTRACT(QUARTER FROM c.current_lost_at)::int
        ELSE NULL
      END AS proposed_fiscal_bucket_str
    FROM classified c
  )
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'current_fiscal_bucket', current_fiscal_bucket_str,
    'proposed_fiscal_bucket', proposed_fiscal_bucket_str,
  ...
  SELECT jsonb_agg(row_to_json(t)) INTO bbp_json FROM (
    ...
    FROM public.deals d
    WHERE lower(coalesce(d.company,'')) NOT LIKE 'test %' AND lower(coalesce(d.company,'')) NOT LIKE 'test-%'
  ...
  VALUES (rep_user_id, auth.uid(), result) RETURNING id INTO run_id;
