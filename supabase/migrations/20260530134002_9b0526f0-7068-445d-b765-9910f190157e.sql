...
BEGIN
  -- 1) Insert a placeholder claap_recordings row for every claap_meeting whose
  --    claap_id has no corresponding recording (scoped per company).
  WITH ins AS (
  ...
  -- 2) Insert primary_meeting link rows that join each meeting to its recording.
  WITH ins AS (
...
GRANT EXECUTE ON FUNCTION public.backfill_claap_recordings_from_meetings() TO service_role;