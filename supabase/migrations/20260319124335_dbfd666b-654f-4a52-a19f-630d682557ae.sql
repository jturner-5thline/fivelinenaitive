...
-- Add deal_types JSONB column to company_settings
ALTER TABLE public.company_settings ADD COLUMN IF NOT EXISTS deal_types jsonb DEFAULT NULL;

-- Update the seed function to also seed deal types
...
BEGIN
  ...
  -- ─── 1. Company Settings (deal stages, data room checklists, deal types) ───
  SELECT EXISTS (
  ...
  IF NOT has_settings THEN
    INSERT INTO public.company_settings (
      ...
      '[
        ...
        {"id":"bridge-loan","label":"Bridge Loan"}
      ]'::jsonb,
      '{
        "version": 2,
        "configs": [
