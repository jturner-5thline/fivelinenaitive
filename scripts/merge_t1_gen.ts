...
for(const g of idx.groups){
  ...
  if(!tiers.every(t=>t==='T1')){
    if(tiers.some(t=>t==='T1')) skipped.push({groupId:g.groupId, members:m.map(x=>({name:x.name,tier:x.tier}))});
    continue;
  }
...
await Bun.write('/tmp/merge_t1.sql', lines.join('\n'));
await Bun.write('/tmp/merge_t1_report.json', JSON.stringify({groups:t3.length, deleted:t3.reduce((s,g)=>s+g.dups.length,0), skipped, t1_clusters: t3.map(g=>({canonical:g.canonical.name, dups:g.dups.map(d=>d.name)}))}, null, 2));
